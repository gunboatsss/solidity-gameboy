...
abstract contract GbMemory is GbStorage {
    ...
    function _loadRom(bytes memory data) internal {
        rom = data;
        _initState();
    }

    /// @dev Storage path: allocates memories, detects MBC, resets CPU.
    ///      Short ROMs are used as-is (reads past the end return 0xFF).
    function _initState() internal {
        uint256 len = rom.length;
        require(len > 0x149, "ROM too small");
        delete romStores;
        romLen = 0;
        useSstore2 = false;
        _allocAndReset(uint8(rom[0x147]), uint8(rom[0x149]));
    }

    /// @dev SSTORE2 path: same, but ROM lives in code blobs + prefetch buffer.
    function _initSstore2() internal {
        require(romLen > 0x149, "ROM too small");
        require(romStores.length > 0, "no stores");
    ...
    function _loadFrame(Frame memory fr) internal view {
        ...
        if (useSstore2) fr.romBuf = _fetchRom();
    }
    ...
    function _flushFrame(Frame memory fr, uint32 executed) internal {
        regA = fr.a;
        regA = fr.a;
        regF = fr.f & 0xF0;
        regB = fr.b;
    ...
    function _romByte(Frame memory fr, uint256 off) internal view returns (uint8) {
        ...
        if (off >= rom.length) return 0xFF;
        return uint8(rom[off]);
    }
    ...
    function _writeIO(Frame memory fr, uint8 lo, uint8 v) internal {
        unchecked {
            if (lo == 0x00) {
                ...
                fr.sc = v;
                if (v == 0x81) {
                    // instant serial transfer: capture byte (Blargg output)
                    serialOut.push(bytes1(fr.sb));
