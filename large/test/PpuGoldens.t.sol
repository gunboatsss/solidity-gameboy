...
/// @notice Golden framebuffer battery, large build (core readMem, large slots).
///         States are poked directly into storage (slots verified by
///         test_SlotSanity + `FOUNDRY_PROFILE=large forge inspect
///         large/GameBoy.sol:GameBoy storage-layout`), then rendered with
///         one runFrame. Hashes below were captured from the reference
///         Solidity renderer — any renderer change must reproduce them
///         exactly.
...
contract PpuGoldensTest is Test {
    ...
    // storage slots (verify with `FOUNDRY_PROFILE=large forge inspect large/GameBoy.sol:GameBoy storage-layout`)
    uint256 internal constant VSLOT = 8;
    uint256 internal constant OSLOT = 10;
    uint256 internal constant IOSLOT = 12;
    // IO byte offsets within slot 12 (little-endian packing: byte N = value << 8N)
    ...
    function _fresh() internal returns (GameBoy gb) {
        ...
        gb = factory.create();
        gb.loadRomStoreChunk(_head(rom));
        gb.loadRomStoreChunk(_tail(rom));
        gb.finalizeSstore2Load();
    }

    function _head(bytes memory rom) internal pure returns (bytes memory o) {
        o = new bytes(0x4000);
        for (uint256 i = 0; i < 0x4000; i++) o[i] = rom[i];
    }

    function _tail(bytes memory rom) internal pure returns (bytes memory o) {
        o = new bytes(0x4000);
        for (uint256 i = 0; i < 0x4000; i++) o[i] = rom[0x4000 + i];
    }
