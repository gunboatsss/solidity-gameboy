...
/// @dev Shared revert reasons (custom errors keep bytecode small).
error RomTooSmall();
error NoStores();
error BadMbc();
error BadBlob();
...
abstract contract GbStorage {
    ...
    // ---- Cartridge / MBC1 (SSTORE2 ROM only in slim build) ----
    // bank-aligned 16KB code blobs (store k = file bytes [k*0x4000, ...));
    // prefetched into Frame.romBuf on every call.
    address[] romStores;
    uint256 romLen;
    bool useSstore2;
    ...
    struct Frame {
        ...
        bool bankMode;
        uint32 frameCycles;
        // SSTORE2 ROM prefetched here on every call
        bytes romBuf;
